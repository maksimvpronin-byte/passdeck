...
export type TouchIdAvailability = {
  available: boolean;
  reason?: string;
};

export interface TouchIdSecretStore {
  availability(): TouchIdAvailability;
  hasStoredPassword(filePath: string): Promise<boolean>;
  storePassword(filePath: string, password: string): Promise<void>;
  getPassword(filePath: string): Promise<string>;
  forget(filePath: string): Promise<void>;
}

export class ElectronTouchIdSecretStore implements TouchIdSecretStore {
  availability(): TouchIdAvailability {
    if (process.platform !== 'darwin') {
      return { available: false, reason: 'Touch ID доступен только на macOS.' };
    }
    if (!safeStorage.isEncryptionAvailable()) {
      return { available: false, reason: 'safeStorage недоступен.' };
    }
    if (!systemPreferences.canPromptTouchID()) {
      return { available: false, reason: 'Touch ID недоступен или не настроен в macOS.' };
    }
  ...
  async hasStoredPassword(filePath: string): Promise<boolean> {
    try {
